...
class KnowledgeExtractor(nn.Module):
    def __init__(self, embd_dim, output_dim, num_layers, device):
        super().__init__()
        self.device = device
        self.position_encoding = PositionalEncoding(d_model=embd_dim).to(device)
        encode_layer = nn.TransformerEncoderLayer(d_model=embd_dim, nhead=8, batch_first=True).to(device)
        self.encoders = nn.TransformerEncoder(encode_layer, num_layers=num_layers).to(device)
        self.classifier = nn.Linear(embd_dim, output_dim).to(device)
...
class KnowledgeEmbedding(nn.Module):
    def __init__(self, n_embd, embd_dim, output_dim):
        super(KnowledgeEmbedding, self).__init__()
        self.knowledge_embedding = nn.Embedding(n_embd, embd_dim)
        encoder_layer = nn.TransformerEncoderLayer(d_model=embd_dim, nhead=8, batch_first=True)
        self.knowledge_encoder = nn.TransformerEncoder(encoder_layer, num_layers=1)
        self.output_layer = nn.Linear(embd_dim, output_dim)

    def forward(self, x):
        x = self.knowledge_embedding(x)
        x = self.knowledge_encoder(x)
